...
Overfit monitor: F1 gap (train - test / train - cv), threshold from config
...
def predict_proba_positive(pipeline, X) -> np.ndarray:
    ...
    if hasattr(pipeline, "decision_function"):
        ...
        # Squash unbounded scores to (0, 1) for ranking metrics.
...
def log_and_save_model(pipeline, metrics, model_path, model_type, X_example, params=None):
    ...
    # Explicit input/output schema for the sklearn predict() contract (class label).
    pred_labels = np.asarray(pipeline.predict(input_example)).astype(int)
    signature = infer_signature(input_example, pred_labels)

    # Document probability output separately (used by API risk scoring later).
...
def train_evaluate_and_log(
    ...
    # Explicit CV-prefixed copies for gate/dashboard clarity.
    metrics_cv.update(
        {
            "f1_cv": metrics_cv["f1"],
    ...
    metrics_train = calculate_classification_metrics(
        y_train, train_pred, train_proba, prefix=""
    )
    ...
    metrics_test = calculate_classification_metrics(
        y_test, test_pred, test_proba, prefix=""
    )
    metrics_test = {f"{k}_test": v for k, v in metrics_test.items()}

    overfit_f1_gap = float(
        max(0.0, metrics_train["f1_train"] - metrics_test["f1_test"])
    )
    optuna_score, overfit_cv_gap = build_optuna_score(
        metrics_cv["pr_auc_cv"],
        metrics_train["f1_train"],
        metrics_cv["f1_cv"],
    )
...
def main():
    ...
    if args.model_type == "h2o":
        raise SystemExit(
            "H2O AutoML is deferred for the classification migration. "
            "Use sklearn/XGBoost model types instead."
        )

    # 1. Temporal data preparation (no random split).
    X_train, X_test, y_train, y_test, cleaned_df = prepare_pipe_break_data()
    # Avoid MLflow integer+missing schema enforcement issues at inference time.
            ...
            def objective(trial):
                params = {
                    ...
                    "max_depth": trial.suggest_int(
                        "max_depth", args.depth_min, args.depth_max
                    ),
                ...
                with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True):
                    ...
                    cv_pred = (cv_proba >= 0.5).astype(int)
                    cv_metrics = calculate_classification_metrics(
                        y_train, cv_pred, cv_proba, prefix=""
                    )
                    ...
                    log_mlflow_data(
                        params={**params, "model_type": args.model_type},
    ...
    # Silence unused-variable lint for cleaned_df while keeping the API parity.
    _ = cleaned_df
